...
g_ip_range = args['ip']
cidr_pat = (r"^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|"
            r"[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(\/([0-9]|[1-2][0-9]|"
            r"3[0-2]))$")
...
def give_ips(ip_cidr):
    ...
    for i in range(start, end):
        yield socket.inet_ntoa(struct.pack('>I', i)) + '/' + str(cidr)
...
run_cmd("vrouter-interface-add vrouter-name %s-vrouter ip %s vlan %s if "
        "data" % (g_switch_list[0], ip1, g_vlan_id))

time.sleep(2)
print("")

intf_info = run_cmd("vrouter-interface-show vrouter-name %s-vrouter ip %s "
...
run_cmd("vrouter-interface-add vrouter-name %s-vrouter ip %s vlan %s "
        "if data vrrp-id %s vrrp-primary %s vrrp-priority %s" % (
            g_switch_list[0], vip, g_vlan_id, g_vrrp_id, pintf_index,
            g_prim_vrrp_pri))
...
run_cmd("vrouter-interface-add vrouter-name %s-vrouter ip %s vlan %s "
        "if data" % (g_switch_list[1], ip2, g_vlan_id))
...
run_cmd("vrouter-interface-add vrouter-name %s-vrouter ip %s vlan %s if data "
        "vrrp-id %s vrrp-primary %s vrrp-priority %s" % (
            g_switch_list[1], vip, g_vlan_id, g_vrrp_id, sintf_index,
            g_sec_vrrp_pri))
